...
export default function RegistrySection({ guitars }: Props) {
  ...
  const gen1 = guitars.filter(g => g.generation === 'Gen 1').length
  const gen2 = guitars.filter(g => g.generation === 'Gen 2').length
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr
                ...
                onClick={() => window.location.href = `/guitar/${g.mgr_id}`}
              >
                <td style={{ padding: '13px 14px', color: '#c8a96e' }}>{formatMgrId(g.mgr_id)}</td>
                <td style={{ padding: '13px 14px', color: '#f0ede8' }}>{g.serial ?? '—'}</td>
                <td style={{ padding: '13px 14px', color: '#f0ede8' }}>{g.model ?? '—'}</td>
                ...
                <td style={{ padding: '13px 14px', color: '#9e9b96' }}>
                  {g.factory_colour ? g.factory_colour.split(' — ')[0] : '—'}
                </td>
                <td style={{ padding: '13px 14px', color: '#9e9b96' }}>
                  {[g.last_known_city, g.last_known_country].filter(Boolean).join(', ') || '—'}
                </td>
                <td style={{ padding: '13px 14px', color: '#9e9b96' }}>
